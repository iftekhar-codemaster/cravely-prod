...
- Media & Distance UI: Cloudflare R2 uploaded logos are stored on `restaurant.logo`; all card and profile components prioritize `restaurant.logo || restaurant.image` rendered through `SmartImg`. Geolocation distance is handled reactively by `<RestaurantDistance>` using `useUserLocation()` with `fallbackKm`, gracefully showing `— km` when distance is indeterminate (preventing `0 km` anomalies).
